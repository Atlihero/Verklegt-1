...
class PlayerIO:
    ...
    def save_players(players: list[Player]):
        ''' This function takes and saves a player which has been created 
        by using the csv writer to save him and he now is in the 
        players csv with all the details he needs'''
    ...
    def create_new_player(player: list):
        '''This functions is used to write a new player into the csv file 
        by using the csv writer'''
        try: 
            with open(PLAYER_PATH, "a",newline="", encoding="utf-8") as csvfile:
            # Thehe user writes in what is needed for the player like name, DOB etc.
                writer = csv.writer(csvfile) 

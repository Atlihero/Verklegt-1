...
class LLOrganizer():
    ...
    def choose_start_date(self, start_date: str) -> datetime:
        ...
        try:
			...
			# Check if inputted date is in the future, if date not in the future then invalid date
    ...
    def tournament_location(self, location: str) -> str:
        '''Checks if there is a location for the tournament'''
    ...
    def tournament_contact_name(self, contact_name: str) -> str:
        '''Checks if the name is missing'''

        contact_name = contact_name.strip()
        if not contact_name: # Check for empty input
            raise ValueError("Contact name cannot be emtpy. Please enter a name for the contact person.")
        return contact_name

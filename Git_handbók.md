...
-	Notum git til að:
  -	Fylgjast með hvaða breytingar hver og einn gerir í skrám
  -	Vinna með branches til að vera ekki að breyta aðalkóðanum fyrr en við erum viss um allt sé rétt, áður en kóðanum er push-að í aðalkóðann
...
- Main –> alltaf nýjasta útgáfan af kóðanum með öllu réttu
- Develop – þar sem nýr kóði er þróaður
- Feature branches - fyrir nýja virkni
- Release branches – til að undirbúa nýja útgáfu
- Hotfix branches – til að laga alvarlegar villur í kóðanum
- Hjálpar til við að halda skipulagi í stórum erkefnum
...
-	Sýnir stöðu breytinga
  -	Hvort að skrár eru untracked, modified eða staged
-	Gefur yfirlit yfir núverandi stöðu í möppunni sem verið er að vinna í

**Git log**
-	Birtir commit söguna fyrir repo-ið
  - Hvaða commits hafa verið gerð
  - Hver gerði hvaða commit
  - Skilaboðin fyrir commitið
...
- To fork a repo þýðir að búa til sína eigin útgáfu af repo sem annar er með á sínu GitHub
  - Gerir þér kleyft að gera breytingar á því án þess að hafa áhrif á upprunalega skjalið
...
- Ferlið þar sem aðrir hópmeðlimir skoða pull request-ið og koma með athugasemdir (ef það eru einhverjar) áður en breytingarnar eru sameinaðar.
  - Hjálpar við að halda gæðunum í kóðanum og að eitthvað sem á ekki að fara í aðalkóðann fari í hann.

**Árekstrar**
- Árekstrar (conflict) geta komið upp ef breytingar hafa verið gerða á sömu línum í sömu skrám
- Dæmi:
  - Main inniheldur línuna: print(“Hello world”)
  - Feature branch breytir línunni í: print(“Hello Git”)
  - Þegar við reynum að merge-a þá veit Git ekki hvaða útgáfu hann á að velja
  - Lausn: opna skránna og ákveða hora línunna við viljum halda, eða báðar. Vista og commit-a síðan rétta lausn
...
-	**git statu**s: Checking the current state of the working directory and staging area.
-	**git commi**t: Saving staged changes to the repository with a descriptive message.
...
-	**git merg**e: Combining changes from different branches.
